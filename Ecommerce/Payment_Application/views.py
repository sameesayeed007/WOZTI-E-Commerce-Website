...
def payment(request):
    ...
    store_id = 'abc5ef6ff3925681'
    API_key = 'abc5ef6ff3925681@ssl'
    mypayment = SSLCSession(sslc_is_sandbox=True, sslc_store_id=store_id, sslc_store_pass=API_key)

    status_url = request.build_absolute_uri(reverse("Payment_Application:complete"))
    #print(status_url)
...
def complete(request):
    if request.method == 'POST' or request.method == 'post':
        ...
        if status == 'VALID':
            ...
            return HttpResponseRedirect(reverse("Payment_Application:purchase", kwargs={'val_id':val_id, 'tran_id':tran_id},))
        elif status == 'FAILED':
            messages.warning(request, f"Your Payment Failed! Please Try Again! Page will be redirected!")

    return render(request, "Payment_Application/complete.html", context={})
